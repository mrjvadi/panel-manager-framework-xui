...
func (m *Manager) As[T any](id string) (T, bool) {
    var zero T
    slot, ok := m.getSlot(id); if !ok || !slot.enabled { return zero, false }
    v, ok := any(slot.drv).(T); if !ok { return zero, false }
    return v, true
}

// Panel: API روان‌تر به‌صورت fluent
type PanelHandle struct { id string; m *Manager }
...
func (m *Manager) MustAs[T any](id string) (T, error) {
    if v, ok := m.As[T](id); ok { return v, nil }
    var zero T; return zero, ErrExtNotSupported
}

// Try: الگوی اجرایی تمیز برای یک افزونه
func (m *Manager) Try[T any](id string, fn func(T) error) error {
    v, ok := m.As[T](id); if !ok { return ErrExtNotSupported }
    return fn(v)
}
