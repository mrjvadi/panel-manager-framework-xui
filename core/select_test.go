...
func (d dummyDriver) CreateUser(ctx context.Context, u User) (User, error) {
	return u, ErrNotImplemented
}
func (d dummyDriver) UpdateUser(ctx context.Context, u User) (User, error) {
	return u, ErrNotImplemented
}
...
func (d dummyDriver) CreateInbound(ctx context.Context, in Inbound) (Inbound, error) {
	return in, ErrNotImplemented
}
func (d dummyDriver) UpdateInbound(ctx context.Context, in Inbound) (Inbound, error) {
	return in, ErrNotImplemented
}
...
func TestVersionFilters(t *testing.T) {
	...
	if len(got) != 2 {
		t.Fatalf("expected 2 xui, got %v", got)
	}

	got2 := m.VersionEq("v0.8.4").IDs()
	if len(got2) != 1 || got2[0] != "c" {
		t.Fatalf("want c, got %v", got2)
	}
}
