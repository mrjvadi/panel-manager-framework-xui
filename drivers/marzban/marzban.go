...
func New(sp core.PanelSpec, opts ...core.Option) (core.Driver, error) {
    ...
    if sp.Endpoints["login"] == "" { sp.Endpoints["login"] = "/api/admin/token" }
    if sp.Endpoints["listUsers"] == "" { sp.Endpoints["listUsers"] = "/api/admin/users" }
    if sp.Endpoints["system"] == "" { sp.Endpoints["system"] = "/api/system" }
    ...
    if cfg.Logger != nil { cli.Log = cfg.Logger }
...
func (d *driver) Login(ctx context.Context) error {
    ...
    if err := core.DoJSON(ctx, d.cli, req, &out); err != nil { return err }
    if t, ok := out["access_token"].(string); ok { d.stat.token = t }
    return nil
}
...
func (d *driver) ListUsers(ctx context.Context) ([]core.User, error) {
    ...
    if err := d.doJSON(ctx, req, &body); err != nil { return nil, err }
    var out []core.User
    if arr, ok := body["users"].([]any); ok {
        for _, it := range arr {
            if m, ok := it.(map[string]any); ok {
                u := core.User{}
                if s, ok := m["username"].(string); ok { u.Username = s }
                if v, ok := m["up"].(float64); ok { u.Up = int64(v) }
                if v, ok := m["down"].(float64); ok { u.Down = int64(v) }
                out = append(out, u)
            }
        }
    }
...
func (d *driver) SystemInfo(ctx context.Context) (map[string]any, error) {
    ...
    var out map[string]any
    if err := d.doJSON(ctx, req, &out); err != nil { return nil, err }
    if v, ok := out["version"].(string); ok { d.runtimeVersion = v }
    return out, nil
}
