...
func main() {
	...
	_ = mgr.AttachByKind(core.PanelSpec{
		ID:      "marz-1",
		BaseURL: "https://bots.saitsazs.ir:2053",
		Auth:    core.BasicAuth{Username: "Et_MrJavdi", Password: "Et_MrJavdi"},
	...
	for id, users := range usersAll {
		fmt.Println("panel:", id, "users:", len(users))
	}
	...
	go func() {
		...
		_ = mgr.AttachByKind(core.PanelSpec{
			ID:      "xui-s-1",
			BaseURL: "http://91.107.190.148:6522/P7HKO0xVHMfklTv",
			Auth:    core.BasicAuth{Username: "admin", Password: "admin"},
			TLS:     core.TLS{Insecure: true},
			Version: "v2.6.1",
	...
	go func() {
		...
		mgr.Disable("marz-1")
		fmt.Println("disabled marz-1")
		time.Sleep(2 * time.Second)
		mgr.Enable("marz-1")
		fmt.Println("enabled marz-1")
	}()
